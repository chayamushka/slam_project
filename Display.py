...
class Display:
    ...
    #     im_dis = cv2.drawKeypoints(img.get_image(), img.kp, outImage=np.array([]), color=(255, 0, 0),
    #                                flags=cv2.DRAW_MATCHES_FLAGS_NOT_DRAW_SINGLE_POINTS)
    #     Display.end(im_dis, "kp_" + txt, save)
    #
    # @staticmethod
    # def kp_two_color(im: Image, kp0, kp1, txt="kp_2_color.jpg", save=False):
    #     im_dis = cv2.drawKeypoints(im.get_image(), kp0, outImage=np.array([]), color=(0, 165, 255),
    #                                flags=cv2.DRAW_MATCHES_FLAGS_NOT_DRAW_SINGLE_POINTS)
    ...
    #     img_matches = cv2.drawMatches(pair.img0.get_image(), kp0, pair.img1.get_image(), kp1,
    ...
    def scatter_2d(points1, points2=None, points3=None, label=None):
        ...
        None if points3 is None else ax.scatter(points3[:, 0], points3[:, 1], c='green',linewidths=0.1)
