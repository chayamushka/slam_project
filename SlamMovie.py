...
class SlamMovie:
    ...
    def transformation(self, idx):
        ...
        R, t = self._max_supporters_RANSAC(idx, matches, 100)
    ...
    @staticmethod
    def pnp(pair1, pair2, K, matches, idx):
        ...
        _, rvec, tvec = cv2.solvePnP(points_3d, pixels, K, np.zeros((4, 1), dtype=np.float64))
        R = cv2.Rodrigues(rvec)[0]
        return R, tvec.squeeze()

    def _pnp(self, idx, matches, ind):
    ...
    @staticmethod
    def findSupporters(pair1, pair2, K, m, matches, R, t):
        ...
        for i, match in enumerate(matches):
            ...
            left0 = thresh(SlamCompute.projection(point, K), pair1.img0.get_kp_pt(match.queryIdx))
            right0 = thresh(SlamCompute.projection(point + m, K), pair1.img1.get_kp_pt(match.queryIdx))
            left1 = thresh(SlamCompute.projection(point, K, R, t), pair2.img0.get_kp_pt(match.trainIdx))
            right1 = thresh(SlamCompute.projection(point + m, K, R, t), pair2.img1.get_kp_pt(match.trainIdx))
            supporters[i] = left0 and left1 and right0 and right1
        return supporters

    def _findSupporters(self, idx, matches, R, t):
        thresh = lambda ptx, px: np.all(np.abs(ptx - px) <= 2)
        supporters = np.full(len(matches), True)
    ...
    @staticmethod
    def max_supporters_RANSAC(objectCam, relativeCam, k, m1, matches, max_loop=10000, min_loop=200, num_of_point=6,
                              p=0.99):
    ...
    def _max_supporters_RANSAC(self, idx, matches, max_loop=10000, min_loop=200, num_of_point=6, p=0.99):
        num_max_supporters = 0
        ransac_size = 0
        count_loop = 0
        best_R, best_t = np.eye(3), np.zeros(3)
        ...
        while ransac_size + min_loop - count_loop > 0 and count_loop < max_loop:
            ind = np.random.choice(len(matches), size=num_of_point, replace=len(matches) < num_of_point)
            try:
                R, t = self._pnp(idx, matches, ind)
                # R, t = self.pnp(self.img_pairs[idx], self.img_pairs[idx + 1], self.K, matches, max_supporters)
                supporters = self.findSupporters(self.img_pairs[idx], self.img_pairs[idx + 1], self.K, self.stereo_dist,
                                                 matches, R, t)
                if sum(supporters) > num_max_supporters:
                    num_max_supporters, max_supporters = sum(supporters), supporters
                    best_R, best_t = R, t
        ...
        while True:
            try:
                R, t = self._pnp(idx, matches, max_supporters)
                # R, t = self.pnp(self.img_pairs[idx],self.img_pairs[idx+1], self.K,matches, max_supporters)
                # supporters = self._findSupporters(idx, matches, R, t)
                supporters = self.findSupporters(self.img_pairs[idx], self.img_pairs[idx + 1], self.K, self.stereo_dist,
                                                 matches, R, t)
                if sum(supporters) > num_max_supporters:
                    num_max_supporters, max_supporters = sum(supporters), supporters
